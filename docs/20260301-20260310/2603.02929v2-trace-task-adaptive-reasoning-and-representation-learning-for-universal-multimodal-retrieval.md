...
## 摘要
通用多模态检索需要统一的嵌入模型，能够解释从简单关键词到复杂组合指令的各种用户意图。虽然多模态大语言模型（MLLMs）具备强大的推理能力，但现有的适配方法将其局限于静态编码器，未能充分利用其生成潜力。这种仅编码器的范式在处理需要逻辑推导而非表面模式匹配的复杂意图时面临挑战。为此，我们提出了 TRACE（任务自适应推理与压缩嵌入）。TRACE 将生成式推理与判别式表示学习相统一。它首先生成结构化的思维链（CoT）以对查询进行显式推理，随后通过专用标记将该推理轨迹压缩为紧凑的嵌入。为了训练该框架，我们构建了 M-BEIR-CoT，这是一个具有难度感知路由策略的大规模数据集。在 M-BEIR 基准测试上的实验证明 TRACE 达到了新的最先进水平。至关重要的是，TRACE 展示了一种习得的隐式路由行为：它能自主地为复杂查询激活推理，同时跳过简单查询的推理，从而在检索准确率和推理吞吐量之间实现最佳平衡。此外，通过内化演绎过程，TRACE 在未见领域和新约束条件下表现出卓越的零样本迁移能力。
