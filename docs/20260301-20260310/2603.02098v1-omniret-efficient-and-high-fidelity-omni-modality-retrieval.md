...
tags: ["query:main"]
score: 7.0
evidence: 跨文本、视觉和音频的多模态检索
tldr: OmniRet是首个支持文本、视觉和音频三种模态组合查询的检索模型。为解决计算效率和表示保真度问题，它引入了注意力重采样机制以压缩Token序列，并采用注意力切片Wasserstein池化保留细粒度细节。该模型在600万对数据上训练，在13个检索任务中表现卓越，并推出了全新的音频中心多模态基准ACM。
selection_source: fresh_fetch
motivation: 现有多模态检索模型大多局限于文本和视觉，无法处理涉及音频等更多模态的复杂组合查询。
method: 提出了注意力重采样机制以提高计算效率，并利用注意力切片Wasserstein池化技术来增强全模态表示的保真度。
result: 在组合查询、音频和视频检索任务上实现了显著的性能提升，并在多个标准基准测试中达到领先水平。
conclusion: OmniRet通过高效的架构设计和全面的基准测试，为构建通用的全模态检索系统奠定了基础。
---
