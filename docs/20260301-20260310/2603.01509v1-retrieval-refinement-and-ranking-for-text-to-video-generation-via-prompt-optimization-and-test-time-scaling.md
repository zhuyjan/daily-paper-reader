...
## 摘要
尽管大规模数据集推动了文本到视频（T2V）生成模型的显著进展，但这些模型对输入提示词仍高度敏感，这表明提示词设计对生成质量至关重要。当前改进视频输出的方法通常存在不足：它们要么依赖复杂的后期编辑模型，面临引入伪影的风险；要么需要对核心生成器进行昂贵的微调，这严重限制了可扩展性和可访问性。在这项工作中，我们提出了 3R，一种基于检索增强生成（RAG）的新型提示词优化框架。3R 利用了当前最先进的 T2V 扩散模型和视觉语言模型的能力。它可以在无需任何模型训练的情况下与任何 T2V 模型配合使用。该框架利用了三个关键策略：基于 RAG 的修饰符提取以增强上下文基础，基于扩散的偏好优化以使输出符合人类偏好，以及时间帧插值以产生时间一致的视觉内容。这些组件共同实现了更准确、高效且上下文对齐的文本到视频生成。实验结果证明了 3R 在增强生成视频的静态保真度和动态连贯性方面的有效性，强调了优化用户提示词的重要性。
